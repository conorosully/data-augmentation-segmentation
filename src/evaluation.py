...
class Args:
    def __init__(self, target_pos, incl_bands, satellite,scale):
...
def get_preds(model, test_paths, batch_size=10, target_pos=-1, incl_bands=[0, 1, 2, 3, 4, 5, 6], satellite="landsat",scale =True,perm_bands=None,device='mps'):
    """Get model predictions for a given dataloader"""
    
    
    args = Args(target_pos, incl_bands, satellite,scale)
